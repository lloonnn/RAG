...
SAMPLE_TEXT = '''# Introcution to Machine Learning  
...
# Solution: Use overlap to prevent retrieve only partial context, because overlap might preserve some previous context
def overlap_importance():
    ...
    # With overlap
    with_overlap = RecursiveCharacterTextSplitter(chunk_size=50, chunk_overlap=10)

    chunks_no_overlap = no_overlap.split_text(text)
    chunks_with_overlap = with_overlap.split_text(text)

    print("Wihout overlap:")
...
'''
The strip() method in Python returns a copy of a string with all leading and trailing 
whitespace characters (spaces, tabs, and newlines) removed
'''